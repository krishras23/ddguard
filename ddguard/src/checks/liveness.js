...
const scopeOf = (parsed) => Object.entries(parsed.scope).map(([k, v]) => `${k}:${v}`).join(', ');

const hasNumericPoint = (series, fromMs) =>
  (series.pointlist || []).some(([ms, value]) => ms >= fromMs && Number.isFinite(value));
...
async function run(monitor, parsed, client) {
  ...
  let series;
  try {
    const res = await client.query(parsed.dataQuery, from, to);
    series = res.series || [];
  } catch (err) {
  ...
  const live = series.filter((s) => hasNumericPoint(s, from * 1000));

  if (!live.length) {
    return [finding('fail', 'NO_POINTS', `Query returned ${series.length} series over the last 24h but not one numeric point — this monitor can never fire.`, {
      detail: `metric: ${parsed.metric}`,
      suggestion: `${parsed.metric}{${scopeOf(parsed)}} has stopped reporting — the series exists, the data does not.`,
    })];
  }

  if (parsed.groupBy.length && live.length > MAX_GROUPS) {
    return [finding('warn', 'TOO_MANY_GROUPS', `Query returns ${live.length} groups — every one is an independent alert, so a shared outage pages ${live.length} times.`, {
      detail: `by {${parsed.groupBy.join(', ')}}`,
      suggestion: 'Drop a grouping key, or aggregate and alert on the total.',
    })];
  }

  return [finding('pass', 'HAS_SERIES', `Returns ${live.length} series over the last 24h.`, {
    detail: `${live.length} series`,
  })];
}
